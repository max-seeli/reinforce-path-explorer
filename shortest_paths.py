...
class ShortestPaths:
    ...
    def __init__(self, map_file = os.path.join(os.getcwd(), "maps/map1.txt")):
        """
    ...
    def store_all_paths(self, map_files, json_file = "maps/opt_path.json"):
    ...
    def calc_shortest_paths(self):
        ...
        results = dict(file = self.map_file, optimal_path_lengths = [], start_positions = [], shortest_paths = [])
        ...
        for start in self.start_nodes:
            ...
            results["start_positions"].append([G.nodes[start]["x_coord"], G.nodes[start]["y_coord"]])
            results["optimal_path_lengths"].append(len(path)-1)
            results["shortest_paths"].append([(G.nodes[i]["x_coord"], G.nodes[i]["y_coord"]) for i in path])
    ...
    def build_graph(self):
        ...
        for (x,y), value in np.ndenumerate(self.grid):
            ...
            if value == CELL.TARGET:
                ...
                for z in range(len(self.velocities)):
                    ...
                    G.add_node(node_index, x_coord=x, y_coord = y, vx = self.velocities[z][0], vy = self.velocities[z][1], z=z)
        ...
        for node_index in G.nodes:
            ...
            neighbors = self.calc_neighbor_indices((node["x_coord"], node["y_coord"]), node["z"])
    ...
    def enumerate_node(self, x,y,z):
        ...
        """
        return x*self.grid.shape[1]*len(self.velocities) + y*len(self.velocities) + z + 1
